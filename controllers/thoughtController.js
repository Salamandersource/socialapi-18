...
module.exports = {
  ...
  deleteThought(req, res) {
    Thought.findOneAndRemove({ _id: req.params.thoughtId })
      .then((thought) => (!thought ? res.status(404).json({ message: "No such thought exists" }) : User.findOneAndUpdate({ users: req.params.userId }, { $pull: { user: req.params.thoughtId } }, { new: true })))
  ...
  createReaction(req, res) {
    Thought.findOneAndUpdate({ _id: req.params.thoughtId }, { $addToSet: { reactions: req.body } }, { runValidators: true, new: true })
      .then((reaction) => (!reaction ? res.status(404).json({ message: "No thought found with that ID :(" }) : res.json(reaction)))
